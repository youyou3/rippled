...
{
    recent_type::time_point const expired (
        cache.clock().now() - Tuning::liveCacheSecondsToLive);
    for (auto iter (cache.chronological.begin());
        iter != cache.chronological.end() && iter.when() <= expired;)
