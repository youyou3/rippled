                              ...
                              SHAMapSyncFilter* filter)
{
    ...
    bool blocking = false; // Are we currently blocking on I/O
    ...
    while (1)
    {
        ...
        // Try a non-blocking pass first
        // If that either finds at least one missing entry or
        // confirms there are no missing entries, we are done
        do
        {
            ...
            while (currentChild < 16)
            {
                ...
                if (!node->isEmptyBranch (branch))
                {
                    ...
                    if (! m_fullBelowCache.touch_if_exists (childHash))
                    {
                        ...
                        if (blocking)
                            d = getNodePointerNT (childID, childHash, filter);
                        else
                            d = getNodePointerNB (childID, childHash, filter, pending);
                        ...
                        if (!d)
                        {
                            ...
                            { // We didn't wait for the database
                                anyDefer = true;
                            }
        ...
        // If we blocked on I/O, no nodes were deferred, or
        // we found at least one missing node we're done
        if (blocking || !anyDefer || !nodeIDs.empty ())
            break;
